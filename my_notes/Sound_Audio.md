...
# 3 Complex number
The Fourier section below will be buit on the Euler format. Thus we need to understand how this format work first
## 3.1 Definition
In short, a complex number is the [extension](https://en.wikipedia.org/wiki/Complex_number) of a real number. One main idea of it is to allow at least a solution to ***all*** polynomial equations, which is impossible in real numbers. Complex number z is defined as
$$z = a+i.b$$
where
- $a,b\in\mathbb{R}$
- $z\in \mathbb{C}$
...
Example for the equation $x^2= -1$ doesn't have any solution in real numbers because $\sqrt{-1}$ is not defined. The condition of x must be that $x\geq 0$. However, complex numbers allow a solution to this by defining that 
$$\sqrt{-1} =i$$  
...
- The Y Axis is called Imaginary Axis, denoted as Im(z)
- The X Axis is called the Real Axis, denoted as Re(z)

For a complex number $x = a+bi$, it is plotted as a point (a, b). Example z = 4+3i, then express the point (4,3)
## 3.3 Modulos and argument
<img width="566" height="421" alt="Image" src="https://github.com/user-attachments/assets/65bce20c-73d1-4806-ac16-e16abf8b3e48" />

- **Modulos**: is the absolute value of the complex number. On the complex plane, it is the distance from the origin to the point it resides. Using pytherogran we have
$$|z| = \sqrt{a^2 + b^2}$$ 
- **Argument**: It is the angle between the real axis and the real number vector
$$\theta = \tan^{-1}\left(\frac{b}{a}\right)$$
## 3.4 Rotation with complex number
As seen in the section above, the modulos is the same regardless of a or b is the real or imginary. Only in **argument** do we need to distinguish between them. So what happen if we change the sign of two parts or switch them together? We could infer that the modulos is still the only the argument is changing. This mean they are litterly on the circle with the radius of modulos. So question is what kind of changing is that? 

Given that we have 
$$z=a+bi$$
Let call $z_{-1},z_{i},z_{-i}$ be complex numbers are we multiply by its corresponding subscripts -1, i, and -i, respectively. Then we have
$$z_{-1} = -a-bi $$ 

$$z_{i} = ai-b = -b + ai$$

$$z_{-i} = -ai+b = b - ai$$
Plot it on graph we see that
...
- **Multiply with i**: This rotate the vector by 90 counterclockwise
- **Multiply with -i**: this rotate the vector by 90 clockwise

## 3.5 Polar form
Another way of representing the complex number is to use Polar form. This mean that instead of using fix coordination (x,y) along the axes, we could use the $(r,\theta)$ where r is the radius and $\theta$ is the argument or angle. This is useful in the case where the case involve of dealing with rotation. Based on section above we would have
$$r = |z| = \sqrt{x^2+y^2}$$

$$\theta = \tan^{-1}\left(\frac{b}{a}\right)$$
<img width="577" height="457" alt="Image" src="https://github.com/user-attachments/assets/443c03c3-2f9f-4c95-a737-eb3b02b07629" />
Based on image above where we project the our vector on both Real and Imaginary axes, we could switch from
$$ z = a + bi$$
to 
$$ z = r\cos(\theta) + ir\sin(\theta) = r\left[\cos(\theta) + i\sin(\theta)\right]$$

- If you fix r and change the $\theta$, the position of complex number is on the circle with the radius r with the center at origin
- If you fix $\theta$ and change r, the position of complex number is on a straight line
Together you could represent any number by rotate it to the direction of the vector and scale it by changing the r
...
# 4 Euler formula
## 4.1 Definition
In short Euler formula is written as 
$$ e^{i\theta} = \cos(\theta) + i \sin(\theta)$$
as you can see that this formula establish the relationship between natural expornent and the trignometry of complex number. From the complex number we could infer its maginute that 
$$r = |z| = \sqrt{(cos(\theta))^2 + (sin(\theta))^2} = \sqrt{1} = 1$$
A fix magnitude means that as we increase the x, we basically just rotate around the origin with radius 1. If there is coefficient $r_0$ before exponent
$$r_0 e^{ix}$$
Then we would have
$$ r_0e^{ix} = r_0\cos(\theta) + i r_0\sin(\theta)$$

$$=>r = |z| = \sqrt{(r_0cos(\theta))^2 + (r_0sin(\theta))^2} = \sqrt{r_0^2} = |r_0|$$
This mean that it would rotate around complex plane with radius as $|r_0|$
## 4.2 Angular frequency
Usually, we don't have theta already for us to rotate around. We usually work with time domain so what we have is the time variable. Therefore, it is natural that our $\theta$ might be depends on the time parameter.
$$e^{i\theta(t)}$$
This mean that as the time increase, the angle $\theta$ also changes. However, majority of the time we only want it to have constant speed. We call this speed $\omega$. THus we will have
$$e^{i\omega t}$$
Depend on what unit we use, for a complete circle, we would have $360\degree$ degree or $2\pi $ radian. I would stick with the radian for now. Since our speed is constant, we only need to know the angle and the time it need to reach that angle. Let call period $T$ be the time needed to complete a circle. Then we would have
$$\omega = \frac{2\pi}{T}$$
Notice the meaning of period is the time needed to complete a circle. Assume that the cycle itself does have an meaning(only in this example), we would have something like this
$$T= \frac{k\ Time}{1\ Circle}$$
This mean that the time needed to complete a unit of circle is $k$. If we flip them we would have
$$\frac{1}{T}=\frac{1\ Circle}{k\ Time} = \frac{k'\ Circle}{1\ Time}$$
This mean that the number of circle complete it one unit of time is $k'$. Or simpler term, how many circles for one second. This is the idea of frequency
$$f= \frac{1}{T}$$
So we would have
$$\omega = 2\pi f$$
And 
...
As seen in the section 3.4 where we could rorate a vector by multiply with some numbers, now I'll try to complex represent those number using euler format
####  Rorate $\pi$ radian: -1
we have
$$z=-1=-1+0i$$
...
$$\theta = \pi$$
So re-write it in polar form and convert it to euler form 
$$z=\cos(\pi)+i\sin(\pi)$$
Using euler formular
$$z=e^{i\pi}$$
...
We can see that the angle they rorate match with $\theta$ specify in exponent formula. This mean that if we arbitrary complex number $z_2$, then we could rotate it like this
> [!NOTE]
>$$z.(-1) = z.e^{i\pi}$$
$$z.(i) = z.e^{i\frac{\pi}{2}}$$
$$z.(-i) = z.e^{-i\frac{\pi}{2}}$$

Turn out that not only Euler can be used as another form for a complex number, it can be used as a means to rotate other vector. Generally, multiply a complex number z with $e^{i\theta}$ will rotate its vector by the angle of $\theta$ 
> [!IMPORTANT]
> $$\textbf{Rotate z by $\theta$:  }ze^{i\theta}$$
### 4.3.2
Above i have shown an application of euler exponent is to rotate a complex number when multiply it. Now if we have a range of number of functions. How would it look like? First we would plot the constant first. Consider an real function
$$y = f(t)$$
Since this real function so the imaginary part is zero
$$y = f(t) + 0i$$
We could calculate its modulo and argument
$$|y| = \sqrt{[f(t)]^2 + 0}=|f(t)|$$

$$\theta = tan^{-1}\left(\frac{0}{f(t)}\right) = 0$$
We see that real function always reside in the real axis regardless of its value. Now we apply the rotation with euler exponent on its for $\theta$ angle.
$$f(t)e^{i\theta}$$

$$=f(t)[\cos(\theta)+i\sin(\theta)]$$

This is the polar form that we shows above. THis mean that
$$r = |f(t)|$$

$$\theta = \theta$$
Well this look kinda obvious but this show that 
The radius is only affected by real function while argument is only affected by Euler exponent in 
$$z = f(t)e^{i\theta}$$

However, since euler exponent control the angle, we would have another straight line of values if the $\theta$ is fixed. Our idea is to make it rotate over the time, so we assign its a constant speed with frequenncy $f$. Thus our formula could re-write as
> [!IMPORTANT]
> Given the function
$$z(t) = f(t)e^{i2\pi f t}$$
This function rotate around the complex plane. The radius is only affected by real function $f(t)$ while the rotation is only affected by Euler Exponent $e^{i2\pi f t}$
 
Now we know that what is the meaning of that multiplication. Next I would like to visualize you of how some functions $f(t)$ is rotated
#### 4.3.2.1 Constant
